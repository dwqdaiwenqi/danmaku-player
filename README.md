...
### 你想问
#### 为什么不要在全屏模式下使用
当调用特效指令，如#下雪后，播放器的渲染就不使用`<video/>`了，而是使用`<canvas/>`获取webgl对象进行渲染。在全屏模式下，WebGl渲染了面积过大的实时视频纹理导致了fps的下降，目前无解（有好的解决方法的务必告知~）
#### 移动端没做处理吗
对，自动播放和全屏的问题移动浏览器没解决，暂时不考虑兼容移动端
#### 视频的缩略图怎么获得
下载个ffmpeg软件，然后执行命令 ``,就生成缩略图了
